"""RC3 wire (5 Sep 2026): as rc2 but g2 composites from the v2 shrinkage build,
plus the balanced nine-member companion daioe_g2nine.

Wire the v1.1.0 release candidate, revision 2 (4 Sep 2026, ML decision):
legacy-membership genai base + G2 + G2GEN + the two activated columns through the
...
SRC = ROOT / "data/vintage/vintage_2025_genailegacy_20260904"
G2DIR = ROOT / "data/vintage/g2_20260824"          # activation columns (raw, sigma-free)
G2V2 = ROOT / "data/vintage/g2v2_20260905"        # shrinkage-sigma composites
DST = ROOT / "data/vintage/vintage_2025_v110rc3_20260905"
REP = ROOT / "reports/vintage_2025_v110rc3_20260905"
NEW = ["g2all", "g2gen", "g2nine", "agentic", "mathsci"]
...
g2 = pd.read_parquet(G2V2 / "g2all_panel_onet.parquet")[["occ_code_onet", "year", "g2all_change"]]
g2 = g2.rename(columns={"g2all_change": "exp_change_g2all"})
def activated(tag):
...
g2g = pd.read_parquet(G2V2 / "g2gen_panel_onet.parquet")[["occ_code_onet", "year", "g2gen_change"]]
g2g = g2g.rename(columns={"g2gen_change": "exp_change_g2gen"})
g2n = pd.read_parquet(G2V2 / "g2nine_panel_onet.parquet")[["occ_code_onet", "year", "g2nine_change"]]
g2n = g2n.rename(columns={"g2nine_change": "exp_change_g2nine"})
onet_new = g2.merge(g2g, on=["occ_code_onet", "year"], how="outer")
onet_new = onet_new.merge(g2n, on=["occ_code_onet", "year"], how="outer")
onet_new = onet_new.merge(activated("agentic"), on=["occ_code_onet", "year"], how="outer")
...
sys.argv = ["restore_frozen_pctl", str(PUB)]
...
sys.argv = ["add_midrank_pctl", str(PUB)]
runpy.run_path(str(ROOT / "scripts/add_midrank_pctl_20260825.py"))

(REP / "RELEASE-v110rc.md").write_text(
    "# v1.1.0 release candidate, revision 3 (wired 5 Sep 2026)\n\n"
    "Base: vintage_2025_genailegacy_20260904. Supersedes rc2. Changes: the\n"
    "second-generation composites rebuilt with shrinkage sigmas (g2_sigma_v2.csv,\n"
    "K=5; scripts/build_g2_v2_20260905.py), the balanced nine-member companion\n"
    "daioe_g2nine added, and the 5 Sep Epoch frontier recompute verified null\n"
    "(provenance_epoch_recompute_2026-09-05.json). Added columns, all taxonomies:\n"
    "daioe_g2all, daioe_g2gen, daioe_g2nine (full window), daioe_agentic and\n"
    "daioe_mathsci (chained 2024), each with pctl_rank_ and pctl_mid_ companions.\n"
    "G2 checks, member shares and leave-one-out tables: reports/g2v2_20260905.\n\n"
