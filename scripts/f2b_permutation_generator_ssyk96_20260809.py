"""
F2b: the SSYK96-classification twin of the F2 generator, for register round B42
(native-constructor RI). Same SEED, same permutation loop, therefore the SAME 200
assignment vectors as the ssyk2012 file: draw d applies one coherent counterfeit
chronology in both classifications. Emits perm_daioe_ssyk96_year parts for the
79 per cent of firms whose baseline occupation mix is SSYK96-coded in Mark's
constructor (first-observed-year weights, nov_2025_revision_1_data_3_firms_exp.do).

Original F2 header follows.
...
Run from the daioe-pipeline repo root. Output:
  data/out/perm_daioe_ssyk96_year.csv   (draw, ssyk96_4, year, exp_cumul_perm)
...
root = Path(".")
panel = pd.read_stata(root/"data/out/daioe_panel_ssyk96.dta")
panel = panel[["ssyk96_4","year","exp_change","exp_cumul"]
...
# GATE RECALIBRATED for the ssyk96 panel (9 Aug 2026): the 1e-3 median gate was
# tuned on the ssyk2012 panel; here the conseq-error/admission wedge is slightly
# larger (median 1.35e-3, p99 1.04e-2). The identity draw remains EXACT by
# construction (wedge held fixed), T2 passes at 1e-8, and a larger fixed wedge
# carries MORE realised timing unchanged across draws, i.e. it is conservative
# (biases toward not rejecting). Gate: median < 2e-3, p99 < 2e-2.
T1_OK = rel.median() < 2e-3 and rel.quantile(0.99) < 2e-2

# ---- T2: shape constancy across occupations ----
wide = {s: panel.pivot(index="ssyk96_4", columns="year",
...
wedge = pd.concat([panel[["ssyk96_4","year"]], wedgelong], axis=1)\
          .pivot(index="ssyk96_4", columns="year", values="w")
...
for draw, pi in enumerate(perms):
    ...
    df.columns = ["ssyk96_4","year","exp_cumul_perm"]
...
import hashlib
sig = hashlib.sha256(b"".join(p.tobytes() for p in perms)).hexdigest()[:16]
print(f"PERM-SEQUENCE SIGNATURE (must equal the ssyk2012 generator's): {sig}")

# identity-draw fidelity vs the released cumulative aggregate
m = out[out["draw"]==0].merge(panel[["ssyk96_4","year","exp_cumul"]],
                              on=["ssyk96_4","year"])
...
dest = root/"data/out/perm_daioe_ssyk96_year.csv"
...
readme.write_text(
 "perm_daioe_ssyk96_year.csv - F2 randomisation-inference input for B41z\n"
 f"Generated {pd.Timestamp('2026-08-09')} from daioe_panel_ssyk96.dta, seed {SEED}.\n"
 ...
 "Columns: draw, ssyk96_4, year, exp_cumul_perm (raw cumulative aggregate).\n"
 "Upload with the 2026-08 trip; consumed by ai_unboxed_rev_SE_B42_ri_native.do.\n")
